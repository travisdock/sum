...
RSpec.describe EntriesController, type: :controller do
  describe 'GET #index' do
    ...
    it 'creates an entry with no tag' do
      ...
      expect {
        post :create,
             params: { entry: { category_id: @category.id, amount: 100, date: Date.today, notes: 'Test',
                                tag_attributes: { name: '' } } }
    ...
    it 'updates an entry with no tag' do
      ...
      expect {
        patch :update,
              params: { id: entry.id,
                        entry: { category_id: @category.id, amount: 200, date: Date.today, notes: 'Test',
                                 tag_attributes: { name: '' } } }
    ...
    it 'updates an entry and removes the existing tag without destroying it' do
      ...
      expect {
        patch :update,
              params: { id: entry.id,
                        entry: { category_id: @category.id, amount: 200, date: Date.today, notes: 'Test',
                                 tag_attributes: { id: tag.id, name: tag.name, _destroy: 1 } } }
      }.to change(Entry, :count).by(0)
       .and change(Tag, :count).by(0)
      expect(Entry.last.amount).to eq(200)
