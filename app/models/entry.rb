class Entry < ApplicationRecord
  ...
  scope :by_year, -> (year) { where("strftime('%Y', date) = ?", year) if year.present? }
  scope :by_month, -> (month) { where("ltrim(strftime('%m', date), '0') = ?", month) unless month.blank? || month.to_i.zero? }
